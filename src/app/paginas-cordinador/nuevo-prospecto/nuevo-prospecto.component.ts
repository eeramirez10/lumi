...
export class NuevoProspectoComponent implements OnInit, AfterViewInit {
  ...
  onSubmit() {
    ...
    if (this.prospectForm.invalid) {
      console.log('invalido', this.ff.select3.value);
      return;
    } else {
  ...
  guardarprospecto() {
    console.log(this.ff.nombre.value, this.addresstext.nativeElement.value, this.ff.contacto.value, this.ff.telefono.value,this.ff.select3.value);

    const headers: any = new HttpHeaders({ 'Content-Type': 'application/json' });
    const options: any = { 'caso': 15, 'idUs': this.ff.select3.value, 'Namepros': this.ff.nombre.value, 'Dirpros': this.addresstext.nativeElement.value, 'rfc': this.ff.rfc.value, 'Contacpros': this.ff.contacto.value,'Correopros': this.ff.correo.value, 'Telpros': this.ff.telefono.value, 'fecha': this.fecha, 'asigno': this.idUsuario};
      ...
      respuesta => {
        if (this.archivos.length > 0) {
          ...
          Swal.fire('El prospecto se envio correctamente', '', 'success')
            .then((value) => {
              setTimeout(() => {
                this.router.navigate(['/Prospectos_coordinador/Nuevo_prospecto']);
  ...
  guardarprospecto2() {
    if (this.ff.select3.value=='') {
      ...
      const headers: any = new HttpHeaders({ 'Content-Type': 'application/json' });
      const options: any = { 'caso': 15.1, 'idUs': this.ff.select3.value, 'Namepros': this.ff.nombre.value, 'Dirpros': this.addresstext.nativeElement.value, 'fecha': this.fecha };
      const URL: any = this.baseURL + 'detalle_cita.php';
      this.http.post(URL, JSON.stringify(options), headers).subscribe(
        respuesta => {
          if (this.archivos.length > 0) {
