...
export class NuevaTareaComponent implements OnInit {
  ...
  opcionSeleccionadoemp: string = "";
  verSeleccionemp: string = "";
  textarea: string = "";
  vertarea: string = "";
  archivos: string[] = [];
  ...
  obteneridequipo() {
    ...
    this.http
      ...
      .subscribe((respuesta) => {
        ...
        this.idequipodetrabajo = this.equipo.length
          ? this.equipo[0].Equipos_idEquipos
          : null;
        console.log(this.idequipodetrabajo);
        this.usuariosequipo(this.idequipodetrabajo);
      });
  }
  ...
  guardartarea() {
    ...
    const options: any = {
      caso: 13,
      idEmp: this.iddeSelectEmpresa,
      suc: this.verSeleccion,
      idRes: this.verSeleccion2,
      tarea: this.vertarea,
      fecha: this.fechat,
      idUs: this.idUsuario,
    };
  ...
  capturar() {
    ...
    this.nombresucursal =
      selectnombremes.options[selectnombremes.selectedIndex].text;
  ...
  capturar1($event, nombre) {
    ...
    if (this.iddeSelectEmpresa === '0') {
      this.dprospectos = true;
    }
    this.dprospectos = false;
  ...
  capturar4() {
    this.idempresatarea = this.ff.select4.value;
    if (this.ff.select4.value == 0) {
      this.dempresas = true;
    }
    this.dempresas = false;
    console.log(this.idempresatarea);
  }
  ...
  servidorarchivo(idtarea) {
    ...
    this.subirarchivo.enviararchivo(formData).subscribe((resp) => {
      if (resp.toString() !== "") {
        Swal.hideLoading();
        Swal.fire("Tarea/cita agregada correctamente", "", "success")
          .then((value) => {
            setTimeout(() => {
              this.router.navigate(['/Tareas_Cordinador']);
            }, 2); // Activate after 2 seconds.
  ...
  obtenerempresas() {
    this.opcionSeleccionado2 == this.idcolaborador;
  ...
  obtenerifoempresa() {
    this.idempresa == this.opcionSeleccionadoemp;
  ...
  crearcita() {
    console.log(
      "direccion " +
        ...
        this.idrecordatorio +
        "empresa" +
        (this.idempresa || this.idprospecto)
    );
    ...
    const options: any = {
      ...
      horai: this.horainicial,
      horaf: this.horafinal,
      idSO: this.opcionSeleccionado,
      idUsuario: this.opcionSeleccionado2,
      recordatorio: this.idrecordatorio,
      idEmpresa: this.iddeSelectEmpresa,
      ...
      .subscribe((respuesta) => {
        ...
        if (res === "0") {
          ...
          if (this.archivos1.length > 0) {
            ...
            this.servidorarchivos(idcita, 0);
          } else {
            this.cerrarmodalcrear.nativeElement.click();
