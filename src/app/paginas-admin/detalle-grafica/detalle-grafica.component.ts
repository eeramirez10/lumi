...
export class Detalle_GraficaComponent implements OnInit {
  ...
  obtenertareas() {
    const headers: any = new HttpHeaders({ 'Content-Type': 'application/json' });
    const options: any = { 'caso': 8.4, 'idEquipo': this.idEq, 'idEmp': this.idEmp };
  ...
  obtenercitas() {
    const headers: any = new HttpHeaders({ 'Content-Type': 'application/json' });
    const options: any = { 'caso': 9.4, 'idEquipo': this.idEq, 'idEmp': this.idEmp };
    const URL: any = this.baseURL + 'perfil.php';
    this.http.post(URL, JSON.stringify(options), headers).subscribe(
      respuesta => {
        this.tareas = respuesta;
